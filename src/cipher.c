...
void Block_permutation(Block *block)
{
    ...
    while(--i >= 0)
    {
        while(--b >= 0)
        {
            permValue = perm[(15 - i) * 8 - b + 7]; //OK
            blockId = 15 - (permValue / 8); //OK
            bitId = permValue - (15 - blockId) * 8;
            newBlock.bundles[blockId] |= (((block->bundles[i] & (1 << (- b + 7))) != 0) << bitId);
            
            //printf("[%d]: %d - blockId: %d - bitId: %d - bitValue: %d\n", (15 - i) * 8 - b + 7, permValue, blockId, bitId, (block->bundles[i] & (1 << - b + 7)) != 0);
        }
        b=8;
    }
    *block = newBlock;
...
void Block_invPermutation(Block *block)
{
    ...
    while(--i >= 0)
    {
        while(--b >= 0)
        {
            permValue = invPerm[(15 - i) * 8 - b + 7]; //OK
            blockId = 15 - (permValue / 8); //OK
            bitId = permValue - (15 - blockId) * 8;
            newBlock.bundles[blockId] |= (((block->bundles[i] & (1 << (- b + 7))) != 0) << bitId);
        }
